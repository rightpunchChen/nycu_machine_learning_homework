...
def save_procedure(Y, labels, iter, save_dir):
    ...
    plt.axis('off')
    plt.savefig(os.path.join(save_dir, f"{(iter):04d}_procedure.png"))
    plt.close()
...
def sne(X=np.array([]), labels=np.array([]), no_dims=2, initial_dims=50, perplexity=30.0, method='t-SNE', save_dir='results'):
    """
    ...
    if round(no_dims) != no_dims:
        ...
        return -1
    if method != 't-SNE' and method != 'sym-SNE':
        print("Error: method should be t-SNE or sym-SNE")
        return -1
    ...
    save_procedure(Y, labels, 0, save_dir)
    ...
    for iter in range(max_iter):
        ...
        # Save procedure
        if (iter + 1) % 20 == 0:
            save_procedure(Y, labels, (iter + 1), save_dir)
...
if __name__ == "__main__":
    ...
    for p in perplexity:
        ...
        # Mack video
        make_video(save_dir, method, p, remove=False)
